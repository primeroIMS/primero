...
module Exporters
  describe SelectedFieldsExcelExporter do
    before :each do
      ...
      form1 = FormSection.new(:name => "cases_test_form_3", :parent_form => "case", "visible" => true,
                             :order_form_group => 0, :order => 0, :order_subform => 0, :form_group_id => "cases_test_form_3",
                             :unique_id => "cases_test_form_3")
      form1.fields << Field.new(:name => "subform_field_2", :type => Field::SUBFORM, :display_name => "subform field", "subform_section_id" => subform.id)
      form1.save!
      #### Build Form Section with subforms fields only ######

      #### Build Form Section with none subforms fields ######
      form2 = FormSection.new(:name => "cases_test_form_2", :parent_form => "case", "visible" => true,
      ...
      form2.fields << Field.new(:name => "array_field", :type => Field::SELECT_BOX, :display_name => "array_field", :multi_select => true,
      ...
      form3 = FormSection.new(:name => "cases_test_form_1", :parent_form => "case", "visible" => true,
      ...
      form3.fields << Field.new(:name => "subform_field_1", :type => Field::SUBFORM, :display_name => "subform field", "subform_section_id" => subform.id)
      form3.save!
      #### Build Form Section with subforms fields and others kind of fields ######
      ...
      @records = [create(:child, "first_name" => "John", "last_name" => "Doe",
                 ...
                 create(:child, "first_name" => "Charlie", "last_name" => "Sheen", "id" => "00000000005",
                           "subform_field_1" => [{"unique_id" =>"21"}]),
                 create(:child, "first_name" => "Emilio", "last_name" => "Steves", "id" => "00000000006",
    ...
    describe 'Export format' do
      ...
      let(:workbook) do
        data = SelectedFieldsExcelExporter.export(@records, @user, {})
    ...
    context 'Selected forms' do
      let(:workbook) do
        data = SelectedFieldsExcelExporter.export(@records, @user, form_unique_ids: %w[cases_test_form_1])
        Spreadsheet.open(StringIO.new(data))
      end
    ...
    context 'Selected fields' do
      let(:workbook) do
        data = SelectedFieldsExcelExporter.export(@records, @user, field_names: %w[first_name array_field])
        Spreadsheet.open(StringIO.new(data))
      end
    ...
    context 'Selected forms and fields' do
      let(:workbook) do
        data = SelectedFieldsExcelExporter.export(
          @records, @user,
