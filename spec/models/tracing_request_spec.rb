...
describe TracingRequest do
  ...
  describe 'build solar schema' do
    ...
    it 'builds with free text search fields' do
      Field.stub(:all_searchable_field_names).and_return []
      TracingRequest.searchable_string_fields.should =~ %w[unique_identifier short_id created_by
                                                           created_by_full_name last_updated_by
                                                           last_updated_by_full_name created_organization
                                                           owned_by_agency_id owned_by_location
                                                           owned_by_agency_office approval_status_assessment
                                                           approval_status_case_plan approval_status_closure
                                                           transfer_status]
    end

    it 'builds with date/time search fields' do
  ...
  xdescribe '.search' do
    ...
    #   tracing_request_active = TracingRequest.create(name: "eduardo aquiles", created_by: 'me',
    #                                                  created_organization: 'stc')
    #   tracing_request_duplicate = TracingRequest.create(name: "aquiles", :duplicate => true, created_by: 'me',
    #                                                     created_organization: 'stc')
    ...
    #   tracing_request_active = TracingRequest.create(name: "eduardo aquiles", :duplicate => false, created_by: 'me',
    #                                                  created_organization: 'stc')
    #   tracing_request_duplicate = TracingRequest.create(name: "aquiles", :duplicate => true, created_by: 'me',
    #                                                     created_organization: 'stc')
    ...
    #   TracingRequest.create(relation_name: 'kev', unique_identifier: '1234567890', last_known_location: 'new york',
    #                         created_by: 'me', created_organization: 'stc')
    #   TracingRequest.create(relation_name: 'kev', unique_identifier: '0987654321', last_known_location: 'new york',
    #                         created_by: 'me', created_organization: 'stc')
  ...
  describe 'new_with_user_name' do
    ...
    it 'should not create a unique id if already exists' do
      tracing_request = create_tracing_request_with_created_by('jdoe', last_known_location: 'London',
                                                                       unique_identifier: 'rapidftrxxx5bcde')
    ...
    it 'should create a posted_at field with the current date' do
      DateTime.stub(:now).and_return(Time.utc(2010, 'jan', 22, 14, 25, 0))
      tracing_request = create_tracing_request_with_created_by('some_user', 'some_field' => 'some_value')
      tracing_request.posted_at.should == DateTime.parse('2010-01-22 14:25:00UTC')
    end
    ...
    describe 'when the created at field is supplied' do
      it 'should use the supplied created at value' do
        tracing_request = create_tracing_request_with_created_by('some_user',
                                                                 'some_field' => 'some_value',
                                                                 created_at: DateTime.parse('2010-01-14 14:05:00UTC'))
        tracing_request.created_at.should == DateTime.parse('2010-01-14 14:05:00UTC')
      end
    end
  end
  ...
  describe 'when fetching tracing request' do
    ...
    it 'should return list of tracing request ordered by name' do
      ...
      expect(tracing_requests.first.relation_name).to eq('Abu')
    end
    ...
    it 'should order tracing_request with blank names first' do
      ...
      tracing_requests = TracingRequest.all.order("data -> 'relation_name' ASC")
      expect(tracing_requests.first.relation_name).to eq('')
      expect(TracingRequest.count).to eq(3)
    end
  end
  ...
  #     #_from is wrong.
  #     tracing_request = create_tracing_request("Bob McBobberson", a_range_field_from: "lkjlj",
  #                                              a_range_field_to: "31-May-2014",
  #                                              a_range_field_date_or_date_range: "date_range")
  #     tracing_request.errors[:a_range_field].should eq(["Please enter the date in a valid format (dd-mmm-yyyy)"])

  #     #_to is wrong.
  #     tracing_request = create_tracing_request("Bob McBobberson", a_range_field_from: "31-May-2014",
  #                                               a_range_field_to: "lk2j3lk45",
  #                                               a_range_field_date_or_date_range: "date_range")
  #     tracing_request.errors[:a_range_field].should eq(["Please enter the date in a valid format (dd-mmm-yyyy)"])

  #     #_from and _to are wrong.
  #     tracing_request = create_tracing_request("Bob McBobberson", a_range_field_from: "lk2j34lkj",
  #                                               a_range_field_to: "akdf34lk4j",
  #                                               a_range_field_date_or_date_range: "date_range")
  #     tracing_request.errors[:a_range_field].should eq(["Please enter the date in a valid format (dd-mmm-yyyy)"])
  ...
  describe 'mother and father' do
    ...
    it 'returns mothers name' do
      expect(@tracing_request1.mothers_name).to eq('Judy')
    end

    it 'returns fathers name' do
      expect(@tracing_request2.fathers_name).to eq('Brad')
    end

    context 'when mother not set' do
      it 'returns nil for mother' do
        expect(@tracing_request3.mothers_name).to be_nil
      end
    end

    context 'when father not set' do
      it 'returns nil for father' do
  ...
  xdescribe 'match criteria' do
    before do
      fields = [Field.new(name: 'name', type: 'text_field', display_name_all: 'name', matchable: true),
                Field.new(name: 'name_nickname', type: 'text_field', display_name_en: 'Nickname', matchable: true),
                Field.new(name: 'sex', type: 'text_field', display_name_en: 'Sex', matchable: true),
                Field.new(name: 'age', type: 'numeric_field', display_name_all: 'Age')]
      TracingRequest.any_instance.stub(:field_definitions).and_return(fields)
      FormSection.create_or_update_form_section(unique_id: 'form_section_test', visible: true, order: 1,
                                                editable: true, fields: fields, parent_form: 'tracing_request',
                                                name_all: 'Form Section With Dates Fields',
                                                description_all: 'Form Section With Dates Fields')
    end
