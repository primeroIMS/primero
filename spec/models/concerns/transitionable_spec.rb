...
describe Transitionable do
  ...
  describe '#all_referrals_have_service_own_entries_permission?' do
    ...
    let!(:referral1) do
      Referral.create!(
        transitioned_by: 'user1', transitioned_to: 'user5', record: @case,
        authorized_role_unique_id: role_with_service_own_entries.unique_id
      )
    end
    let!(:referral2) do
      Referral.create!(
    ...
    let!(:referral3) { Referral.create!(transitioned_by: 'user1', transitioned_to: 'user2', record: @case) }
    let!(:referral4) { Referral.create!(transitioned_by: 'user1', transitioned_to: 'user2', record: @case) }

    context 'when any referrals for a user have an authorized_role with SERVICE_OWN_ENTRIES_ONLY permission' do
      it 'returns true' do
        expect(@case.all_referrals_have_service_own_entries_permission?(@user5)).to be true
      end
    end

    context 'when any referrals for a user do not have an authorized_role with SERVICE_OWN_ENTRIES_ONLY permission' do
      it 'returns false' do
        expect(@case.all_referrals_have_service_own_entries_permission?(@user2)).to be false
      end
    end
  end

  describe '#referrals_without_service_own_entries_permission?' do
    let!(:service_own_entries_permission) do
      Permission.new(
        resource: Permission::CASE,
        actions: [
    ...
    let!(:role_without_service_own_entries) do
      Role.create!(
        name: 'Role without SERVICE_OWN_ENTRIES_ONLY',
        permissions: [permission], modules: [@module_cp]
      )
    end

    let!(:user7) do
      create(
        :user, user_name: 'user7', role: role_with_service_own_entries,
               full_name: 'Test user 7', email: 'user7@primero.dev',
               user_groups: [@group4]
      )
    end
    ...
    let!(:referral1) do
      Referral.create!(
        transitioned_by: 'user1', transitioned_to: 'user7', record: @case,
        authorized_role_unique_id: role_with_service_own_entries.unique_id
      )
    end
    ...
    let!(:referral3) do
      Referral.create!(
        transitioned_by: 'user1', transitioned_to: 'user7', record: case2,
        authorized_role_unique_id: role_without_service_own_entries.unique_id, notes: 'referral3'
      )
    end

    let!(:referral4) do
      Referral.create!(transitioned_by: 'user1', transitioned_to: 'user7', record: case2, notes: 'referral4')
    end

    context 'when any referral have SERVICE_OWN_ENTRIES_ONLY permission' do
      it 'returns true' do
        expect(@case.referrals_without_service_own_entries_permission?(user7)).to be true
      end
    end

    context 'when any referral do not have SERVICE_OWN_ENTRIES_ONLY permission' do
      it 'returns false' do
        expect(case2.referrals_without_service_own_entries_permission?(user7)).to be false
      end
    end
  end
