...
describe ManagedReports::Indicators::CaseViolenceTypeBySexAndAge do
  before do
    ...
    incident5 = Incident.create!(data: { cp_incident_violence_type: 'violence_type_2' })
    incident6 = Incident.create!(data: { cp_incident_violence_type: 'violence_type_3' })

    @case1 = Child.new_with_user(
      @self_user, { registration_date: Date.new(2022, 3, 10), sex: 'female', age: 5, status: 'open' }
    )
    @case1.incidents = [incident1]
    @case1.save!
    @case2 = Child.new_with_user(
      @group_user, { registration_date: Date.new(2023, 4, 18), sex: 'male', age: 10, status: 'open' }
    )
    @case2.incidents = [incident2]
    @case2.save!
    @case3 = Child.new_with_user(
      @agency_user, { registration_date: Date.new(2022, 3, 8), sex: 'male', age: 18, status: 'open' }
    )
    @case3.incidents = [incident3]
    @case3.save!
    @case4 = Child.new_with_user(
      @all_user, {  registration_date: Date.new(2023, 4, 25), sex: 'female', age: 2, status: 'open' }
    )
    @case4.incidents = [incident4, incident5, incident6]
    @case4.save!
  end

  it 'returns data for case_violence_type_by_sex_and_age indicator' do
    report_data = ManagedReports::Indicators::CaseViolenceTypeBySexAndAge.build(nil, {}).data
  ...
  describe 'records in scope' do
    it 'returns records for a self scope' do
      report_data = ManagedReports::Indicators::CaseViolenceTypeBySexAndAge.build(
        @self_user,
        {
          'cp_incident_violence_type' => SearchFilters::Value.new(
            field_name: 'cp_incident_violence_type', value: 'violence_type_1'
          )
        }
      ).data
    ...
    it 'returns group records for a group scope' do
      report_data = ManagedReports::Indicators::CaseViolenceTypeBySexAndAge.build(@group_user, {}).data
    ...
    it 'returns agency records for an agency scope' do
      report_data = ManagedReports::Indicators::CaseViolenceTypeBySexAndAge.build(@agency_user, {}).data
    ...
    it 'returns all records for an all scope' do
      report_data = ManagedReports::Indicators::CaseViolenceTypeBySexAndAge.build(@all_user, {}).data
  ...
  describe 'grouped by' do
    context 'when is year' do
      it 'should return results grouped by year' do
        data = ManagedReports::Indicators::CaseViolenceTypeBySexAndAge.build(
          nil,
          {
