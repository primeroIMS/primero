...
describe Api::V2::SystemSettingsController, type: :request do
  ...
  describe 'GET /api/v2/system_settings' do
    it 'list system_settings when the param *extended* is not present on the request' do
      ...
      expect(json['data']['exact_search_fields'].keys).to eq(%w[cases incidents tracing_requests registry_records
                                                                families])
      expect(json['data']['phonetic_search_fields'].keys).to eq(%w[cases incidents tracing_requests registry_records
                                                                   families])
      expect(json['data']['phone_number_search_fields'].keys).to eq(%w[cases incidents tracing_requests
                                                                       registry_records families])
      expect(
    ...
    it 'lists system_settings and all agencies and all modules when the param *extended* is true on the request' do
      ...
      expect(response).to have_http_status(200)
      expect(json['data'].size).to eq(26)
