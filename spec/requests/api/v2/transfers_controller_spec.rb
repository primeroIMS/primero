...
describe Api::V2::TransfersController, type: :request do
  ...
  before :each do
    ...
    @role = Role.new(permissions_list: [@permission_transfer_case])
  ...
  describe 'GET /api/v2/case/:id/transfers' do
    ...
    before :each do
      @transfer1 = Transfer.create!(transitioned_by: 'user1', to_user_name: 'user2', record: @case)
    end
    ...
    it 'lists the transfers for a case' do
      ...
      expect(json['data'][0]['to_user_name']).to eq('user2')
  ...
  describe 'POST /api/v2/case/:id/transfers' do
    ...
    it 'initiates a transfer of a the record to the target user' do
      sign_in(@user1)
      params = {data: {to_user_name: 'user2', notes: 'Test Notes'}}
      ...
      expect(json['data']['to_user_name']).to eq('user2')
  ...
  describe 'POST /api/v2/case/transfers' do
    ...
    it 'transfer multiple records to the target user' do
      sign_in(@user1)
      params = {data: {ids: [@case.id, @case2.id], to_user_name: 'user2', notes: 'Test Notes'}}
      ...
      expect(json['data'][1]['to_user_name']).to eq('user2')
