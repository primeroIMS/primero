...
describe RecordFlagController do
  ...
  shared_examples_for "Flagging" do
    ...
    it "should flag the record with all parameters" do
      ...
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Testing Flag", "date" => "2014/07/15",
    ...
    it "should flag the record with empty date parameter" do
      ...
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Testing Flag", "date" => "", 
        "flagged_by" => "#{@user.user_name}", "unflag_message" => nil, "removed" => nil,
        "created_at"=>Date.today.strftime("%Y/%m/%d"), "system_generated_followup"=>false})
      record_db = model.get(record.id)
      record_db.flag.should eq(true)
    ...
    it "should flag the record with no date parameter" do
      ...
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Testing Flag", "date" => nil,
       "flagged_by" => "#{@user.user_name}", "unflag_message" => nil, "removed" => nil,
       "created_at"=>Date.today.strftime("%Y/%m/%d"), "system_generated_followup"=>false})
      record_db = model.get(record.id)
      record_db.flag.should eq(true)
  ...
  shared_examples_for "Flagging more than one time" do
    it "should add flag the record with flags already" do
      ...
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Testing Flag 1", "date" => "2014/07/15", 
        "flagged_by" => "#{@user.user_name}", "unflag_message" => nil, "removed" => nil, 
        "created_at"=>Date.today.strftime("%Y/%m/%d"), "system_generated_followup"=>false})

      post :flag, :id => record.id, :model_class => "#{model.name}", :flag_message => "#{model.name} Testing Flag 2", :flag_date => "16-Jul-2014"
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Testing Flag 2", "date" => "2014/07/16", 
        "flagged_by" => "#{@user.user_name}", "unflag_message" => nil, "removed" => nil, 
        "created_at"=>Date.today.strftime("%Y/%m/%d"), "system_generated_followup"=>false})
  ...
  shared_examples_for "Unflagging" do
    it "should remove flag from record" do
      ...
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Flag 2", "date" => nil, 
    ...
    it "should remove flag from record" do
      ...
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Flag 1", "date" => nil, 
      ...
      JSON.parse(response.body).except('unique_id').should eq({"message" => "#{model.name} Flag 3", "date" => nil, 
