...
describe RecordActions do
  context 'permission-based filtering' do
    ...
    describe '#filter_permitted_export_properties' do
      ...
      it "for CSV and read only users call permitted_property_keys" do
        mock_user = double();
        mock_record = double()
      ...
      it "for CSV and update able users call permitted_property_keys" do
        mock_user = double();
        mock_record = double()
        subject.should_receive(:current_user).and_return(mock_user);
        params = ActionController::Parameters.new({:action => "index", :format => "csv"})
      ...
      it "for CSV and create able users call permitted_property_keys" do
        mock_user = double();
        mock_record = double()
        subject.should_receive(:current_user).and_return(mock_user);
        params = ActionController::Parameters.new({:action => "index", :format => "csv"})
      ...
      it "for JSON and call permitted_property_keys" do
        mock_user = double();
        mock_record = double()
        subject.should_receive(:current_user).and_return(mock_user);
