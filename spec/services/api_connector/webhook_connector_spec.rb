...
describe ApiConnector::WebhookConnector do
  before(:each) { clean_data(Role, Field, FormSection, PrimeroModule, RecordSendLog) }
  let(:form) do
    FormSection.create!(
  ...
  describe 'message logging' do
    ...
    it 'logs a success message on a success response' do
      ...
      expect(send_log.destination).to eq(url)
      expect(send_log.status).to eq(RecordSendLog::SENT)
    end
    ...
    it 'logs an error message on a failed response' do
      ...
      send_log = RecordSendLog.first
      expect(send_log.destination).to eq(url)
      expect(send_log.status).to eq(RecordSendLog::FAILED)
    end
  end
end
