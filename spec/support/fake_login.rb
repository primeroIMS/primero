...
module FakeLogin
  ...
  def fake_login_as(resource = nil, actions = [], group_permission = Permission::SELF, user_group_ids = [])
    permission_list = if resource.blank? || actions.blank?
                        Permission.all_available
                      else
                        [Permission.new(resource:, actions:)]
                      end
    user = User.new(user_name: 'fakelimited', user_group_ids:)
    user.stub(:roles).and_return([Role.new(permissions: permission_list, group_permission:)])
    fake_login user
  end
  ...
  def fake_login_with_user_and_permissions(user, permission_list = Permission.all_available,
