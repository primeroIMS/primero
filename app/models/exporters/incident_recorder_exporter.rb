      ...
      def props
         ##### ADMINISTRATIVE INFORMATION #####
        {
          ...
          'service.medical_referral' => ->(model) do
            service_value = model.try(:health_medical_referral_subform_section).try(:first).try(:service_medical_referral)
            incident_recorder_service_referral(service_value) if service_value.present?
          end,
          'service.psycho_referral' => ->(model) do
            service_value = model.try(:psychosocial_counseling_services_subform_section).try(:first).try(:service_psycho_referral)
            incident_recorder_service_referral(service_value) if service_value.present?
          end,
          ...
          'service.legal_referral' => ->(model) do
            service_value = model.try(:legal_assistance_services_subform_section).try(:first).try(:service_legal_referral)
            incident_recorder_service_referral(service_value) if service_value.present?
          end,
          'service.police_referral' => ->(model) do
            service_value = model.try(:police_or_other_type_of_security_services_subform_section).try(:first).try(:service_police_referral)
            incident_recorder_service_referral(service_value) if service_value.present?
          end,
          'service.livelihoods_referral' => ->(model) do
            service_value = model.try(:livelihoods_services_subform_section).try(:first).try(:service_livelihoods_referral)
            incident_recorder_service_referral(service_value) if service_value.present?
          end,
          ##### ADMINISTRATION 2 #####
          'service.protection_referral' => ->(model) do
            service_value = model.try(:child_protection_services_subform_section).try(:first).try(:service_protection_referral)
            incident_recorder_service_referral(service_value) if service_value.present?
          end,
