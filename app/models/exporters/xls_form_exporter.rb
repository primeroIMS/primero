...
class Exporters::XlsFormExporter
  ...
  def dir_name(record_type, primero_module)
    File.join(Rails.root.join('tmp', 'exports'),
              "forms_export_#{record_type}_#{primero_module.name.downcase}_#{DateTime.now.strftime('%Y%m%d.%I%M%S')}")
  end
  ...
  def excel_file_name(file_name = 'default')
    File.join(@export_dir_path, "#{file_name}.xlsx")
  end
  ...
  def export_forms_to_spreadsheet
    ...
    unless @show_hidden_forms
      ...
      visible_subform_ids = visible_top_forms.map { |form| form.fields.map(&:subform_section_id) }.flatten.compact
  ...
  def write_form(form_section)
    ...
    form_headers = %w[type name] + localize_header('label') + localize_header('hint') +
                   localize_header('guidance') + localize_header('tick_box_label')
    @form_sheet.write(0, 0, form_headers)
    choices_header = %w[list name name] + localize_header('label')
    @choices_sheet.write(0, 0, choices_header)
    fields = form_section.fields
    fields = fields.select(&:visible?) unless @show_hidden_fields
  ...
  def write_select(field, type)
    ...
    if field.option_strings_source.present? && field.option_strings_source.start_with?('lookup')
      options = @locales.map { |locale| field.options_list(locale: locale, lookups: nil) }
  ...
  def write_field_row(field, type)
    ...
    field_row += get_localized_property(field, 'help_text')
    field_row += get_localized_property(field, 'guiding_questions')
    field_row += get_localized_property(field, 'tick_box_label')
  ...
  def type_mapping(field)
    ...
    type = (field.multi_select? ? 'select_multiple' : 'select_one') if type == 'select'
    type
  end
  ...
  def compute_locales(input_locales = nil)
    ...
    if input_locales.empty? || correct_locales.empty?
      @locales = all_locales
    else
