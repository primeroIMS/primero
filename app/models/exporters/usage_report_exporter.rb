...
class Exporters::UsageReportExporter < Exporters::ExcelExporter
  ...
  def export(start_date, end_date, request)
    ...
    modules = UsageReportService.modules
  ...
  def get_common_keys(module_id, start_date, end_date)
    ['', UsageReportService.get_total_records(module_id, Child), UsageReportService.get_open_cases(module_id),
     UsageReportService.get_closed_cases(module_id),
     UsageReportService.get_new_records_quarter(module_id, start_date, end_date, Child),
     UsageReportService.get_closed_cases_quarter(module_id, start_date, end_date),
     UsageReportService.get_total_services(module_id), UsageReportService.get_total_records(module_id, Incident),
     UsageReportService.get_new_records_quarter(module_id, start_date, end_date, Incident)]
  end
  ...
  def module_content(module_id, start_date, end_date, modul_name)
    ...
    case modul_name
    when 'MRM'
      ['', UsageReportService.get_total_records(module_id, Incident),
       UsageReportService.get_new_records_quarter(module_id, start_date, end_date, Incident)]
    when 'GBV'
      common_keys
    else
      common_keys + [UsageReportService.get_total_followup(module_id)]
    end
  end
  ...
  def export_user_row(start_date, end_date, request)
    ...
    row_indx = 6
    UsageReportService.modules.each do |modul|
  ...
  def total_agencies
    ['Total No. of Agencies', UsageReportService.all_agencies.count]
  end

  def module_tabs(module_id, modul_name)
    if modul_name == 'MRM'
      [modul_name, UsageReportService.get_total_records(module_id, Incident).positive? ? ' Yes' : ' No']
    else
      [modul_name, UsageReportService.get_total_records(module_id, Child).positive? ? ' Yes' : ' No']
    end
  end
  ...
  def user_content(start_date, end_date)
    data = UsageReportService.all_agencies.map do |agency|
      [agency.unique_id,
       UsageReportService.get_all_users(agency),
       UsageReportService.get_active_users(agency),
       UsageReportService.get_disabled_users(agency),
       UsageReportService.get_new_quarter_users(agency, start_date, end_date)]
    end
    data.transpose
  end
