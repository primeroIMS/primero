...
class Permission < ValueObject
  ...
  RESOURCE_ACTIONS = {
    CASE => [
      ...
      REOPEN, CLOSE, VIEW_PROTECTION_CONCERNS_FILTER, CHANGE_LOG, LIST_CASE_NAMES, MANAGE
    ],
    INCIDENT => [
  ...
  class << self
    ...
    def actions
      [READ, CREATE, WRITE, ENABLE_DISABLE_RECORD, FLAG, INCIDENT_FROM_CASE, INCIDENT_DETAILS_FROM_CASE,
       ...
       DASH_SHARED_WITH_MY_TEAM, DASH_SHARED_WITH_MY_TEAM_OVERVIEW, DASH_CASE_INCIDENT_OVERVIEW,
       RECEIVE_REFERRAL_DIFFERENT_MODULE, LIST_CASE_NAMES, VIOLATION_REPORT]
    end

    def resources
      [CASE, INCIDENT, TRACING_REQUEST, POTENTIAL_MATCH, REGISTRY, ROLE, USER, USER_GROUP, AGENCY, WEBHOOK, METADATA,
       SYSTEM, REPORT, MANAGED_REPORT, DASHBOARD, AUDIT_LOG, MATCHING_CONFIGURATION, DUPLICATE, CODE_OF_CONDUCT,
       ACTIVITY_LOG]
    end

    def records
      [CASE, INCIDENT, TRACING_REQUEST, REGISTRY]
    end
  ...
  def record?
    [CASE, INCIDENT, TRACING_REQUEST, REGISTRY].include?(resource)
  end
