...
class Filter < ValueObject
  ...
  class << self
    ...
    def case_filters(user)
      ...
      filters << DATE_CASE unless user.gbv_or_mrm_user?
      filters << ENABLED
    ...
    def user_based_filters(user)
      ...
      filters << RISK_LEVEL unless user.gbv_or_mrm_user?
      filters << CURRENT_LOCATION unless user.gbv_or_mrm_user?
    ...
    def protection_status_filter(user, filter_fields)
      return [PROTECTION_STATUS] if visible?('protection_status', filter_fields) && !user.gbv_or_mrm_user?

      []
    end

    def urgent_protection_concern_filter(user, filter_fields)
      return [URGENT_PROTECTION_CONCERN] if !user.gbv_or_mrm_user? && visible?('urgent_protection_concern',
                                                                               filter_fields)
    ...
    def type_of_risk_filter(user, filter_fields)
      return [TYPE_OF_RISK] if !user.gbv_or_mrm_user? && visible?('type_of_risk', filter_fields)

      []
    end

    def reporting_location_filters(user)
      return [] if user.gbv_or_mrm_user?

      role = user&.role
      return [] unless role

      filters = []
      filters << REPORTING_LOCATION.call(reporting_location_data(role, 'case')) unless user.gbv_or_mrm_user?
      filters
    end
    ...
    def incident_filters(user)
      ...
      filters += [AGE_RANGE] unless user.module?(PrimeroModule::MRM)
