...
# This describes all models that may indexed and searched in solr
module Searchable
  ...
  included do
    ...
    # The current approach is to gently bounce Puma.
    searchable do
  ...
  module ClassMethods
    ...
    def list_records(filters = {}, sort = { created_at: :desc }, pagination_parms = {}, associated_user_names = [],
                     query = nil, match = nil, owned_by_groups = [])
      self.search do
        build_filters(self, filters) if filters.present?
            ...
            any_of do
              ...
              when 'range'
                values.each do |value|
                  if value.count == 1
                    # Range +
                    with(filter).greater_than_or_equal_to(value.first.to_i)
                  else
                    range_start = value.first.to_i
                    range_stop = value.last.to_i
    ...
    def searchable_boolean_fields
      (%w[duplicate flag has_photo record_state case_status_reopened] +
        Field.all_searchable_boolean_field_names(parent_form)).uniq
    end
    ...
    def searchable_string_fields
      %w[ unique_identifier short_id created_by created_by_full_name
          ...
          owned_by_agency_id owned_by_location owned_by_agency_office ] +
