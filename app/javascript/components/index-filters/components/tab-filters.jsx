...
const TabFilters = ({
  ...
  const filters = useMemoizedSelector(state => getFiltersByRecordType(state, filterCategory));
  const hasPrimeroModuleMRM = useMemoizedSelector(state => hasPrimeroModule(state, MODULES.MRM));
  const allPrimaryFilters = filters.filter(filter =>
    (PRIMARY_FILTERS[filterCategory] || PRIMARY_FILTERS.default).includes(filter.field_name)
  );
