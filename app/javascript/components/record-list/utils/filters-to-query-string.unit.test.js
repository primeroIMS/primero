...
describe("<RecordList />/utils - filtersToQueryString", () => {
  it("returns a query string for the filters object", () => {
    expect(filtersToQueryString(fromJS({ record_state: [true], status: ["open"] }))).toBe(
  ...
  it("returns a query string for a list with hashes", () => {
    ...
    ).toBe("not%5B0%5D%5Buser_name%5D=user1&not%5B1%5D%5Buser_name%5D=user2&status%5B0%5D=open");
  });
  ...
  it("returns a query string for a date range with positive time zone", () => {
    expect(filtersToQueryString(fromJS({ created_at: ["1970-01-01T00:00:00+00:00..2025-02-17T20:04:54+00:00"] }))).toBe(
      "created_at%5B0%5D=1970-01-01T00%3A00%3A00%2B00%3A00..2025-02-17T20%3A04%3A54%2B00%3A00"
    );
  });
});
