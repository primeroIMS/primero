...
describe("<Dashboard />", () => {
  ...
  it("renders the flags", () => {
    expect(component.find(FlagBox)).to.have.length(2);
  });
  ...
  it("renders the Table", () => {
    expect(component.find(DashboardTable)).to.have.length(1);
  });

  it("renders only one TableRow in the TableBody", () => {
    expect(component.find(TableBody).find(TableRow)).to.have.length(1);
  });
});
