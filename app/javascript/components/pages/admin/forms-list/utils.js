...
export const buildOrderUpdater = (currentOrder, newOrder, orderField = "order_form_group") => {
  if (getOrderDirection(currentOrder, newOrder) > 0) {
    return formSection => formSection.set(orderField, formSection.get(orderField) - ORDER_STEP);
  }
...
export const reorderElems = ({ fieldsMeta, orderMeta, elemId, elems }) => {
  ...
  const currentOrder = elems.find(elem => elem.get(idField) === elemId).get(orderField);
...
export const getFormGroupId = (formSections, formUniqueId) =>
  formSections.find(form => form.get("unique_id") === formUniqueId).form_group_id;
