...
const Container = ({ forms, record, recordType, selectedFilters, mobileDisplay, handleToggleNav }) => {
  const i18n = useI18n();
  const css = useStyles();
  ...
  const recordChangeLogs = useMemoizedSelector(state =>
    getChangeLogs(state, record?.get("id"), recordType, forms, selectedFilters)
  );
