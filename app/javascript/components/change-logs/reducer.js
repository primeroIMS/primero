...
const reducer = (state = DEFAULT_STATE, { type, payload }) => {
  switch (type) {
    case actions.FETCH_CHANGE_LOGS_STARTED:
      return state.set("loading", true).set("errors", false);
    case actions.FETCH_CHANGE_LOGS_SUCCESS: {
      const { data, metadata } = payload;
      const newEntries = listEntriesToRecord(data, ChangeLogsRecord);
    ...
    case actions.FETCH_CHANGE_LOGS_FAILURE:
      return state.set("loading", false).set("errors", true);
    case actions.FETCH_CHANGE_LOGS_FINISHED:
