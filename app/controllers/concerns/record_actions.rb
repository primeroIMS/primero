module RecordActions
  ...
  def retrieve_records_and_total(filter)
    ...
    # When mobile is implemented, it should not use 'all'
      ...
      user_names = filters[:user_names]
      user_group_ids = filters[:user_group_ids]
      search = model_class.list_records(filter, order, pagination, user_names, params[:query], params[:match], user_group_ids)
  ...
  def select_permitted_fields(record, permitted_property_names, is_remote_request)
    ...
    default_fields_to_share = is_remote_request ? %w[module_id] : %w[_id _rev couchrest-type histories photo_keys document_keys case_id short_id owned_by created_by registration_date incident_id tracing_request_id inquiry_date incident_links module_id alerts]
