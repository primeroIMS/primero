class HomeController < ApplicationController
  ...
  def display_cases_dashboard?
    @display_cases_dashboard ||= @current_user.modules.select{|m| m.associated_record_types.include?("case")}.any?
  end

  def display_incidents_dashboard?
    @display_incidents_dashboard ||= @current_user.has_module?(PrimeroModule::MRM)
  end
  ...
  def load_incidents_information
    ...
    modules = current_user.modules.select{|m| m.associated_record_types.include?("incident") and m.id == PrimeroModule::MRM}
    ...
    @open_incidents = Incident.list_records(filters={"record_state" => "single||true",
                                                     "module_id" => "single||#{PrimeroModule::MRM}",
                                                     "status" => "single||#{t('dashboard.incident_status_open')}"
                                            }, sort={:created_at => :desc}, pagination={ per_page: 20 }).results
  end

  def get_flags_by_date(model)
    model.by_flag_with_date(:startkey => 1.week.ago.utc).all.uniq(&:id).map {|c| c.flags}.flatten
  end

  def get_recent_record_flagged(model, modules)
    records = []
    modules.each do |primero_module|
      records.concat(model.by_flag_created_at_latest(:startkey => [primero_module.id, {}],
                                              :endkey => [primero_module.id, 1.week.ago.utc],
  ...
  def get_recent_record_flagged_count(model, modules)
    records = 0
    modules.each do |primero_module|
       row = model.by_flag_created_at_latest(:startkey => [primero_module.id, 1.week.ago.utc],
                                      :endkey => [primero_module.id, {}], 
