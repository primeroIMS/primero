...
def generate_searchable_identifiers(record)
  record.generate_searchable_identifiers(
    record.class.phone_number_fields, PhoneticSearchable::IDENTIFIER_TYPE_PHONE_NUMBER
  ).map do |searchable_identifier|
...
models.map(&:constantize).each do |model|
  records_to_process(model, file_path).find_in_batches(batch_size: 1000).with_index do |records, batch|
    ...
    records.each do |record|
      current_size = record.searchable_identifiers.where(
        identifier_type: PhoneticSearchable::IDENTIFIER_TYPE_PHONE_NUMBER
      ).size
