...
def sample_box_muller(lower, upper, mean, standard_deviation)
  ...
  [[lower, mean + scale * Math.cos(theta)].max, upper].min
end

def find_or_create_location(location_code, attributes)
  location = Location.find_or_initialize_by(location_code:)
...
county_level_location =
  if !Location.exists?(admin_level: 2)
    ...
    find_or_create_location('GBR', name: 'United Kingdom', placename: 'United Kingdom', type: 'Country',
                                   hierarchy_path: 'GBR', admin_level: 0)
    find_or_create_location('01', name: 'England', placename: 'England', type: 'Region', hierarchy_path: 'GBR.01',
                                  admin_level: 1)
    find_or_create_location('41', name: 'London', placename: 'London', type: 'County', hierarchy_path: 'GBR.01.41',
                                  admin_level: 2)
  else
